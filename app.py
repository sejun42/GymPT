...
app.config['R2_PUBLIC_URL'] = os.environ.get('R2_PUBLIC_URL', '').rstrip('/')
...
def anal():
    ...
    video_entries = (ExerciseData.query
                     .filter_by(user_id=user_id)
                     .filter(ExerciseData.video_url.isnot(None))
                     .order_by(ExerciseData.date.desc())
                     .limit(6)
                     .all())
    video_urls = [entry.video_url for entry in video_entries if entry.video_url]
...
def upload_video():
    ...
    original_name = secure_filename(video.filename or "exercise_recording.webm")
    _, ext = os.path.splitext(original_name)
    if not ext:
        ext = ".webm"

    r2_key = f"videos/user/{user_id}/{timestamp}_set{set_number}{ext}"
    ...
    video_url = upload_to_r2(temp_path, r2_key, delete_local=True)
    ...
    if video_url and set_number_int is not None:
        exercise_data = (ExerciseData.query
                         .filter_by(user_id=user_id, set=set_number_int)
                         .order_by(ExerciseData.date.desc())
                         .first())
        if exercise_data:
            exercise_data.video_url = video_url
            db.session.commit()
...
def inject_r2_public_url():
    return {"r2_public_url": app.config.get("R2_PUBLIC_URL", "")}
